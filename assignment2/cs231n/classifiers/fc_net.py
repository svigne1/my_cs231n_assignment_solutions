...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        l2_m, cache_l2_m = affine_forward(l1, self.params['W2'], self.params['b2'])
        scores = l2_m
        ...
        ############################################################################
        loss, dl2_m = softmax_loss(scores, y)
        dl1, grads['W2'], grads['b2'] = affine_backward(dl2_m, cache_l2_m)
        dx, grads['W1'], grads['b1'] = affine_relu_backward(dl1, cache_l1)

        loss += 0.5 * self.reg * (np.sum(np.square(self.params['W2'])) + np.sum(np.square(self.params['W1'])) )
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        final = relu_layers + 2
        layer_input, layer_cache[final] = affine_forward(layer_input, self.params['W'+str(final-1)], self.params['b'+str(final-1)])
